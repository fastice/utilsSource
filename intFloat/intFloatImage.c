...
/*
   Clip edges to avoid making them holes.
   if padedges == TRUE, treat border as a hole. Added 2/28/03
*/
...
*/
    ...
    for(i=0; i < na; i++) for(j=0; j < nr; j++) labels[i][j]=0;
/*
...
*/
    ...
    /*  for(i=1; i <= maxLabel; i++) fprintf(stderr,"hole %i %i \n",i,holes[i].nH);*/
/*
    screen Holes
*/
...
*/
     ...
     for(i=1; i < maxLabel; i++) {
	    ...
	    /*            if(holes[i].nH > 100) 
               fprintf(stderr,"Hole %i %i %i %i\n",
	       i,holes[i].nH,holes[i].nB,skip);*/
...
*/
    static void screenHoles(hole *holes, int maxLabel, int thresh, 
                           ...
                           int interpLength,float ratThresh,int allowBreaks)
{
    int i,j;
