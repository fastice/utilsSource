...
static void borderPixels(int32_t lList[9], int32_t **label, int32_t *nLab, int32_t i, int32_t j, int32_t nr, int32_t na, int32_t fastFlag);
static void findPoints(int32_t j, hole holes, intData *data, double *zTmp, double *dTmp, float **image, int32_t *nPts, float radius, float *minD);
static float wDistInterp(intData *data, double *zTmp, double *dTmp,
						 int32_t nPts, float x, float y);

static void planeCoeffs(void *x, int32_t i, double *afunc, int32_t ma);
static void quadCoeffs(void *x, int32_t i, double *afunc, int32_t ma);

static float planeInterp(intData *data, double *zTmp, double *dTmp, int32_t nPts, float x, float y, double **u, double **v, double *w, double a[4], int32_t update);

static float quadInterp(intData *data, double *zTmp, double *dTmp, int32_t nPts, float x, float y, double **u, double **v, double *w, double a[7], int32_t update);

static void clipBorders(unwrapPhaseImage *inputImage, char **mask, double minValue, int32_t *width, int32_t *height);
static void screenHoles(hole *holes, int32_t maxLabel, int32_t thresh, int32_t width, int32_t height, int32_t interpLength, float ratThresh, int32_t allowBreaks);

static int32_t findMaxLabel(int32_t **labels, int32_t nr, int32_t na)
{
	int32_t i, j, maxLabel;
	maxLabel = 0;
	for (i = 0; i < na; i++)
		for (j = 0; j < nr; j++)
			if (labels[i][j] < LARGEINT)
				maxLabel = max(maxLabel, labels[i][j]);
	fprintf(stderr, "1maxLabel = %i \n", maxLabel);
	return maxLabel;
}

static hole *createHoles(int32_t maxLabel)
{
	int32_t i, j;
	hole *holes;
...
static void countHoles(hole *holes, int32_t **labels, char **mask, int32_t nr, int32_t na, int32_t *lList, int32_t nLab, int32_t fastFlag)
{
	int32_t i, j, k;
	for (i = 0; i < na; i++)
		for (j = 0; j < nr; j++)
...
static void saveHoles(hole *holes, int32_t **labels, char **mask, int32_t nr, int32_t na, int32_t *lList, int32_t nLab, int32_t fastFlag)
{
	int32_t i, j, k;
...
/*
  Interpolate floating point32_t image.
*/
					  ...
					  int32_t thresh, float ratThresh, int32_t fastFlag, int32_t allowBreaks)
{
	...
	int32_t xp, yp; /* Hole point32_t where last update occured */
	double a[8];
	...
	int32_t lList[25], nLab; /* List of labels for border point32_t */
	int32_t nPts;
	float minD;
	int32_t skipValue;
	...
	*/
	if (interpLength < 2)
		error("interpolateFloatImage -- interpLenth < 2\n");
	ma = 3;
	...
	*/
	...
	for (i = 0; i < na; i++)
	{
		...
		labels[i] = (int32_t *)malloc(nr * sizeof(int));
		for (j = 0; j < nr; j++)
		{
			mask[i][j] = VALIDPIXEL;
			if (inputImage->phase[i][j] <= minValue)
				mask[i][j] += REGION;
			labels[i][j] = 0;
		}
	}
	if (padEdges == FALSE)
		clipBorders(inputImage, mask, minValue, &width, &height);
	/*
	...
	*/
	...
	for (i = 1; i < maxLabel; i++)
	{
		if (holes[i].nH > 0)
		{
			...
			if (holes[i].nH > 100)
				skip = skipValue; /* find pts every skip pixls */
			for (j = 0; j < holes[i].nH; j++)
			{
				...
				if (nPts > 3)
				{
					if (intType == WDIST || (nPts < 6 && intType == QUADRATIC))
						...
						error("intFloatImage: Invalid interpolation type");
				}
			}
		} /* End if(holes... */
	}	  /* end i */
...
*/
static void screenHoles(hole *holes, int32_t maxLabel, int32_t thresh,
						int32_t width, int32_t height, int32_t interpLength, float ratThresh, int32_t allowBreaks)
{
	int32_t i, j;
...
static float planeInterp(intData *data, double *zTmp, double *dTmp,
						 int32_t nPts, float x, float y, double **u,
						 double **v, double *w, double a[4], int32_t update)
{
	int32_t ma;
	int32_t i;
	double chisq;
...
static float quadInterp(intData *data, double *zTmp, double *dTmp,
						int32_t nPts, float x, float y, double **u,
						double **v, double *w, double a[7], int32_t update)
{
	int32_t ma;
	int32_t i;
	double chisq;
...
static float wDistInterp(intData *data, double *zTmp, double *dTmp, int32_t nPts, float x, float y)
{
	int32_t i;
	double weight;
	double sum;
...
static void findPoints(int32_t j, hole holes, intData *data, double *zTmp,
					   double *dTmp, float **image, int32_t *nPts,
...
*/
static void borderPixels(int32_t lList[9], int32_t **label, int32_t *nLab,
						 int32_t i, int32_t j, int32_t nr, int32_t na, int32_t fastFlag)
{
	int32_t i1, j1, k;
	int32_t exist;
	int32_t nd;
...
static void clipBorders(unwrapPhaseImage *inputImage, char **mask, double minValue, int32_t *width, int32_t *height)
{
	...
	*/
	...
	{
		for (j = 0; j < nr; j++)
			if (inputImage->phase[na1][j] > minValue)
				count++;
		na1++;
	}
	for (i = 0; i < na1; i++)
		for (j = 0; j < nr; j++)
			mask[i][j] &= INVALIDPIXEL;
	...
	{
		for (j = 0; j < nr; j++)
			if (inputImage->phase[na2][j] > minValue)
				count++;
		na2--;
	}
	for (i = na2 + 1; i < na; i++)
	...
	{
		for (i = 0; i < na; i++)
			if (inputImage->phase[i][nr1] > minValue)
				count++;
		nr1++;
	}
	for (i = 0; i < na; i++)
	...
	{
		for (i = 0; i < na; i++)
			if (inputImage->phase[i][nr2] > minValue)
				count++;
		nr2--;
	}
	for (i = 0; i < na; i++)
